...
const (
	FlagsNone     Flags = 0
	LateXFragment Flags = 1 << iota // Don't generate a complete XML document

	CommonFlags Flags = FlagsNone
)

// RendererOptions is a collection of supplementary parameters tweaking
// the behavior of various parts of LaTeX renderer.
type RendererOptions struct {
	...
	// Generator is a comment that is inserted in the generated LaTeX to show what rendered it.
	Generator string
}
...
func (r *Renderer) text(w io.Writer, text *ast.Text) {
	escapeSpecialChars(w, text.Literal)
}
...
func (r *Renderer) heading(w io.Writer, node *ast.Heading, entering bool) {
	...
	case 2:
		r.outs(w, "\\subsection{")
	case 3:
		r.outs(w, "\\subsubsection{")
	case 4:
		r.outs(w, "\\paragraph{")
	case 5:
		r.outs(w, "\\subparagraph{")
	case 6:
	}
}
...
func (r *Renderer) citation(w io.Writer, node *ast.Citation, entering bool) {
	...
	for i, _ := range node.Destination {
...
func (r *Renderer) RenderNode(w io.Writer, node ast.Node, entering bool) ast.WalkStatus {
	...
	case *ast.TableFooter:
		r.outOneOf(w, entering, "<tfoot>", "</tfoot>")
	case *ast.BlockQuote:
