...
const (
	FlagsNone   Flags = 0
	PodFragment Flags = 1 << iota // Don't generate a complete document
...
func (r *Renderer) codeBlock(w io.Writer, codeBlock *ast.CodeBlock, entering bool) {
	if entering {
		// use :text so the =over and =back work to indent this
		r.outs(w, "\n=begin :text\n\n")
		r.outs(w, "\n=over 4\n\n")
		escapeSpecialChars(r, w, codeBlock.Literal)
		r.outs(w, "\n\n=back\n")
		r.outs(w, "\n\n=end :text\n")
		r.outs(w, "\nZ<>\n") // spacing is off in my testing, add extra line
	}
}
...
func (r *Renderer) RenderNode(w io.Writer, node ast.Node, entering bool) ast.WalkStatus {
	...
	case *ast.Emph:
		r.outOneOf(w, entering, "I<", ">")
	case *ast.Strong:
		r.outOneOf(w, entering, "B<", ">")
	case *ast.Del:
...
func (r *Renderer) RenderHeader(w io.Writer, _ ast.Node) {
	if r.opts.Flags&PodFragment != 0 {
		return
	}
	r.outs(w, "=for comment Generated by Mmark Markdown Processer - mmark.miek.nl\n")
	r.outs(w, "=encoding utf8\n")
}
